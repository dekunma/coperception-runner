...
def save_rendered_image():
    folder_name = os.path.join('./logs', 'visualization', f'agent{agent_idx}/scene_{current_scene_idx}')
...
if __name__ == "__main__":
    ...
    data_carscenes = V2XSimSeg(dataset_roots=[data_path],
                                   split=split, config=config, val=True, kd_flag=True, bound='both', no_cross_road=False)
    
    data_carscenes_no_cross_road = V2XSimSeg(dataset_roots=[data_path],
                                   split=split, config=config, val=True, kd_flag=True, bound='both', no_cross_road=True)
    ...
    data_carscenes_det_no_cross_road = V2XSimDet(
            dataset_roots=[data_path_det], split=split, config_global=config_global,
            config=config, val=True, no_cross_road=True, bound='both')
